...
| Bot connects to Discord fine, but every message replies `⚠️ [Errno 2] No such file or directory: 'claude'` | `CLAUDE_BIN` is unset (defaults to `claude`, resolved via `PATH`) and systemd's `PATH` doesn't include `~/.local/bin` — works fine if you run `bridge.py` by hand, fails only under the service | Set `CLAUDE_BIN` to the full path (`which claude` at a normal shell) in the instance's `.env` — `.env.example`/`instances.example.env` ship this uncommented for exactly this reason, so it's usually a case of an older instance file predating that. |
| `PrivilegedIntentsRequired` in `journalctl`, service crash-loops (`NRestarts` climbing) | **Message Content Intent** isn't enabled for that bot in the Discord Developer Portal | Bot tab → Privileged Gateway Intents → enable Message Content Intent → Save, then `sudo systemctl restart discord-bridge@<name>.service`. If it crash-looped enough to hit `StartLimitBurst`, also run `sudo systemctl reset-failed discord-bridge@<name>.service` first. |
